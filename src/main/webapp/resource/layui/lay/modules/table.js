...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function(exports){
    ...
    return ['<table cellspacing="0" cellpadding="0" border="0" class="layui-table" '
      ...
      ,'{{# layui.each(d.data.cols, function(i1, item1){ }}'
        ...
        ,'{{# layui.each(item1, function(i2, item2){ }}'
          ...
          ,'{{# if(item2.checkbox){ }}'
            ,'<th data-field="{{(item2.field?item2.field.replace(\'.\',\'\'):item2.field)||i2}}" data-type="checkbox" {{#if(item2.colspan){}} colspan="{{item2.colspan}}"{{#} if(item2.rowspan){}} rowspan="{{item2.rowspan}}"{{#}}} unresize="true"><div class="layui-table-cell laytable-cell-checkbox"><input type="checkbox" name="layTableCheckbox" lay-skin="primary" lay-filter="layTableAllChoose" {{# if(item2[d.data.checkName]){ }}checked{{# }; }}></div></th>'
          ,'{{# } else if(item2.space){ }}'
            ,'<th data-field="{{(item2.field?item2.field.replace(\'.\',\'\'):item2.field)||i2}}" {{#if(item2.colspan){}} colspan="{{item2.colspan}}"{{#} if(item2.rowspan){}} rowspan="{{item2.rowspan}}"{{#}}} unresize="true"><div class="layui-table-cell laytable-cell-space"></div></th>'
          ,'{{# } else { }}'
            ,'<th data-field="{{(item2.field?item2.field.replace(\'.\',\'\'):item2.field)||i2}}" {{#if(item2.colspan){}} colspan="{{item2.colspan}}"{{#} if(item2.rowspan){}} rowspan="{{item2.rowspan}}"{{#}}} {{# if(item2.unresize){ }}unresize="true"{{# } }}>'
              ,'{{# if(item2.colspan > 1){ }}'
                ,'<div class="layui-table-cell laytable-cell-group" {{#if(item2.align){}}align="{{item2.align}}"{{#}}}>'
                  ,'<span>{{item2.title||""}}</span>'
                ,'</div>'
              ,'{{# } else { }}'
                ,'<div class="layui-table-cell laytable-cell-{{d.index}}-{{(item2.field?item2.field.replace(\'.\',\'\'):item2.field)||i2}}" {{#if(item2.align){}}align="{{item2.align}}"{{#}}}>'
                  ,'<span>{{item2.title||""}}</span>'
                  ,'{{# if(item2.sort){ }}'
  ...
  ,TPL_MAIN = ['<div class="layui-form layui-border-box {{d.VIEW_CLASS}}" lay-filter="LAY-table-{{d.index}}" style="{{# if(d.data.width){ }}width:{{d.data.width}}px;{{# } }} {{# if(d.data.height){ }}height:{{d.data.height}}px;{{# } }}">'
    ...
    ,'{{# layui.each(d.data.cols, function(i1, item1){'
      ,'layui.each(item1, function(i2, item2){ }}'
        ,'.laytable-cell-{{d.index}}-{{(item2.field?item2.field.replace(\'.\',\'\'):item2.field)||i2}}{ width:{{item2.width||50}}px }'
      ,'{{# });'
    ,'}); }}'
    ,'</style>'
  ,'</div>'].join('')
    ...
    ,render = function(){
      ...
      layui.each(data, function(i1, item1){
        ...
        that.eachCols(function(i3, item3){
            var content = "" ;
            if(item3.field && item3.field.indexOf(".")!=-1){
                var propArr = item3.field.split(".");
                var entity = item1[propArr[0]];
                if(entity){
                    content = entity[propArr[1]]
                }
            }else{
                content = item1[item3.field||i3];
            }
          if(content === undefined || content === null) content = '';
          
          if(item3.colspan > 1) return;
          
          var td = ['<td data-field="'+ ((item3.field?item3.field.replace(".",""):item3.field)||i3) +'"'+ function(){
            var attr = [];
            ...
            ,'<div class="layui-table-cell laytable-cell-'+ function(){
              ...
              if(item3.field && item3.field.indexOf(".")!=-1){
                  return options.index + '-' + (item3.field.replace(".","")||i3);
              }else{
              ...
              if(item3.dataDeal){
                return eval(item3.dataDeal + '(item1)');
              }
