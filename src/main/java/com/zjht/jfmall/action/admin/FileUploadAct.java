...
import com.zjht.jfmall.util.FileUtils;
import com.zjht.jfmall.util.ImageUtils;
import com.zjht.jfmall.util.ResponseState;
import com.zjht.jfmall.util.ResponseUtils;
...
public class FileUploadAct {
    ...
    public ResultDto o_delImage(HttpServletRequest request, String fileUrl) {
        try {
            ...
            if (FileUtils.delFile(getSystemUploadDir(fileUrl.replace(serverPath, "")))) {
     ...
     */
    @NoNeedAuth
    @RequestMapping(value = "/file/upload.html")
    public String uploadHtml(HttpServletRequest request, HttpServletResponse response,ModelMap model) {
    ...
    private void uploadFile(HttpServletRequest request, String bizCode, List<JSONObject> fileList) {
        ...
        while (fileItor.hasNext()) {
            ...
            try {
                ...
                fileObj.put("filePath", serverPath + savePath + bizCode + Constants.SPT + retFilePath);
